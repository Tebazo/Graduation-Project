...
%T: De onde saiu esse grupo? Não fizemos nada disso! Substituir este texto por objetivo específico falando da extração de palavras-chave e de comparar a abordagem do keygraph
...
%T: Não sei do que tu tá falando aqui
Visando o objetivo secundário, o processo será similar, porém as palavras chave comparadas serão somente aquelas identificadas nas dissertações, e o objetivo da correlação será o agrupamento daqueles que atuam na mesma área.
...
Os autores sugerem a seguinte parametrização:
\begin{itemize}
\item \(N_{HF} = 30\).
\item \(N_{HK} = 12\).
\item \(N_{KW} = 12\).
...
\end{equation}
Feito isso, são considerados os \(N_{HK}\) termos com maiores valores de \(key\) como \(HK\).
...
%T: Melhorar essa frase aqui
Alguns casos as \textit{foundations} possuem uma leve relação, porém não o suficiente para separá-los.	
...
\begin{figure}[ht!]
\centering
\includegraphics[scale=0.9]{G_com_HK}
...
Os maiores valores de \textit{column} do exemplo foram ('text', 14), ('dad', 9), ('form', 8) e ('estrut', 6). Para conseguir um melhor detalhamento, foi selecionado o \textit{cluster} {'dad', 'text', 'palavr', 'estrut', 'conteud'}, pois no mesmo será perceptível a remoção da aresta de corte entre \textit{palavr} e \textit{conteud}.
...
Os vértices do grafo são ordenados pela soma de valores de \textit{column} que chegam neles. São selecionados os \(N_{HK}\) com os maiores valores de soma, resultando nas palavras-chave.
...
Vale ressaltar que os dois cenários no qual o KeyGraph foi exposto, de uma maneira geral, apresentou pequenas diferença de comportamento. Visto que, em um número de artigos seu desempenho foi superior com o KeyGraph30, como no Artigo 1 no qual o docente foi recomendado na primeira posição enquanto no KeyGraph50 o mesmo caiu para a quinta posição. Por outro lado, no Artigo 2 haviam dois docentes diretamente relacionados ao artigo, com os parâmetros do KeyGraph50 eles foram ranqueados em primeiro e terceiro, em contrapartida eles passaram para terceiro e quarto no KeyGraph30. Em suma, a alteração de parâmetros do KeyGraph, fez com que um número de artigos fosse melhor avaliado, mas por outro lado piorou o desempenho do restante, ou seja, não impactando no resultado geral, uma vez que a nota do algoritmo em ambos os casos foi a mesma.
...
Uma grande dificuldade encontrada no projeto, foi a leitura dos textos presente nos PDFs, uma vez que o modo como o documento foi salvo, alteram-se as configuração do arquivo, e no momento de extração
...
necessário a implementação do método, baseando-se apenas no artigo \cite{ohsawa1998keygraph}.
Muitas
...
pesquisa.
O aumento da coleção dos docentes é um ponto de suma importância, tanto
na questão de quantidade de documentos, como para avaliar o uso da ferramente.
Com a adição de artigos
publicados, orientações de mestrado/doutorado e projetos de pesquisa à coleção, a linha de pesquisa
do docente se manterá sempre atualizada.
Possibilitando uma maior eficácia do algoritmo, uma
